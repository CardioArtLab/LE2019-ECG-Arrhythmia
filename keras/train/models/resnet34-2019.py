...
from keras.layers import (
    Input, Conv1D, BatchNormalization, Activation, Dense, Add, MaxPooling1D,
    Dropout, Reshape, AveragePooling1D, Flatten
)
...
def build_model(**params):
    ...
    layer = BatchNormalization()(layer)
    layer = Activation(params['conv_activation'])(layer)
    #layer = AveragePooling1D(pool=2)(layer)
    layer = Flatten()(layer)
    #layer = Conv1D(256, params['conv_last_size'], padding='valid')(layer)
    output = Dense(params['output_size'], activation='softmax')(layer)
    # customize optimizer
